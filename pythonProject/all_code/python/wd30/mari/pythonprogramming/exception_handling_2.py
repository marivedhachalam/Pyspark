#I am planning to go to office by car
#try:Block level exception handling
 #open the car - if it doesn't open
 #starting the car - if it doesn't start
 #anyother operation in the car fails
#except CarNotOpening -> do something --occurs when any line of code fails
#except CarNotStarting -> do something,call mechanic/leave the car and go by bike
#except Exception -> common issues which was not anticipated occured
#else:optional block --executes only if no exception occured
#I reached office using the car
#finally:optional block (always executes at any cost)
#Ensure the car is parked and locked safely
...
try:#block level exception
    ...
    print(f"file not found: {detailsOfError} , opening default file")
    ...
    print("whether exception occurred or not, finally will be executed")
